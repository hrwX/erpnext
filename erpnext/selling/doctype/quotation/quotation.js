				...
				frm.add_custom_button(__("Authorize"), () => {
					...
					function (data) {
						frappe.call({
							...
							callback: (r) => {
								if (!r.exc) {
									frappe.msgprint(__(`The document {0} has been successfully emailed to {1}.`, [frm.doc.name, data.contact_person]));
								}
							}
						})
					},
		...
		if (!frm.is_new() && frm.doc.docstatus === 1) {
			frm.add_custom_button(__("Contract"), () => {
				frappe.model.open_mapped_doc({
					method: "erpnext.utilities.utils.create_contract_from_quotation",
					frm: frm
				});
			}, __("Create"));
		}
