...
frappe.ui.form.on("Contract", {
	refresh: function (frm) {
		frm.add_fetch("contract_template", "contract_terms", "contract_terms");
		frm.add_fetch("contract_template", "requires_fulfilment", "requires_fulfilment");
		...
		if (frm.doc.docstatus === 1 && !frm.doc.customer_signature) {
			frm.add_custom_button(__("Authorize"), () => {
				...
				function (data) {
					frappe.call({
						method: "erpnext.utilities.utils.create_authorization_request",
						args: {
							dt: frm.doc.doctype,
							dn: frm.doc.name,
	...
	party_name: function (frm) {
		if (frm.doc.party_type == 'Employee' && frm.doc.party_name) {
			frappe.db.get_value("Employee", { "name": frm.doc.party_name }, "employee_name", (r) => {
				frm.set_value("employee_name", r && r.employee_name ? r.employee_name : null);
			});
		} else {
